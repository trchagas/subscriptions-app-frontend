    ...
    <div ref="list_open_tasks" class="row vld-parent">
      ...
      >
        <span title="Ver tarefa" class="task-card pb-1 shadow">
          ...
          <span class="float-right text-sm text-muted" v-if="task.end_date"
...
export default {
  ...
  async mounted() {
    ...
    try {
      const { data } = await api.get(
        `/collaborator/challenges/${this.$props.challengeId}/evaluator/tasks/${this.$props.equipeId}`
      );
  ...
  methods: {
    handleResponseTask(taskId) {
      alert(
        `/colaborador/desafios/${this.$props.challengeId}/avaliador/equipes/${this.$props.equipeId}/tarefas/${taskId}/resposta`
      );
      this.$router.push(
        `/colaborador/desafios/${this.$props.challengeId}/avaliador/equipes/${this.$props.equipeId}/tarefas/${taskId}/resposta/`
      );
