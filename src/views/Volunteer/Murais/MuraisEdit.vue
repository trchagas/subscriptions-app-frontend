  ...
  async mounted() {
    ...
    try {
      const { data } = await api.get(
        `/volunteer/news/${this.$route.params.id}`
      );
      ...
      this.$router.push("/voluntario/comunidade");
    } finally {
      loader.hide();
    }
  },
  methods: {
    async handleUpdate() {
      ...
      try {
        ...
        await api.put(`/volunteer/news/${this.model.id}`, data, config);
    ...
    async handleRemove() {
      ...
      try {
        await api.delete(`/volunteer/news/${this.$route.params.id}`);
