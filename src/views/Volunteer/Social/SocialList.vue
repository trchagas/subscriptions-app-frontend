  ...
  data() {
    return {
      ...
      status: {
        ...
        hidden: {
          tooltip: "Publicação oculta pelo voluntário",
          name: "Oculto",
          type: "primary",
        },
      },
  ...
  methods: {
    async loadPublications(getParams, targetObjName, loaderRef, errorMsg) {
      ...
      try {
        const {
          data: { data },
        } = await api.get(`/volunteer/news/user/${0}?${getParams}`);
    ...
    async handleOpenModalView(news, ref) {
      ...
      try {
        const { data } = await api.get(`/volunteer/news/${news.id}`);
    ...
    openPublicacao(id) {
      this.$router.push(`/voluntario/murais/publicacoes/${id}/editar`);
    },
    openLink(link) {
