...
export default {
  ...
  methods: {
    ...
    async handleDeleteComment(id) {
      try {
        await api.delete(`/volunteer/news/${this.news.id}/comment/${id}`);
    ...
    async handleCommentResponse(id) {
      ...
      try {
        await api.post(`/volunteer/news/${this.news.id}/comment`, {
    ...
    async handleLike() {
      ...
      try {
        await api.put(`/volunteer/news/${this.news.id}/like`);
      } catch (error) {
        this.news.likes -= 1;
        this.news.liked = !this.news.liked;
