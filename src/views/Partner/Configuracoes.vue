              ...
              <div class="row align-items-center">
                <div class="col-12">
                  <h3 class="mb-0">
                    ...
                    <span v-if="model.parceiro.partner_id" class="float-right">
                      #{{
                        model.parceiro.partner_id.toString().padStart(4, "0")
                      }}
                    </span>
                  </h3>
                </div>
                ...
                >
                  ...
                  <div class="col-lg-6">
                    <base-input label="Começou em">
                      <p v-if="model.parceiro.start_at">
                        {{ formatDate(model.parceiro.start_at) }}
                      </p>
                      <p v-else>Não informada</p>
                    </base-input>
                  </div>
...
export default {
  ...
  methods: {
    async handleSubmitForm(e) {
      ...
      try {
        const { data } = await api.put(
          `/partner/partners/${this.model.parceiro.user_id}`,
          parceiroData
        );
        Object.assign(this.model.parceiro, data);
    ...
    async handleSubmitProfilePicture() {
      ...
      try {
        await api.post(
          `/partner/users/${this.model.parceiro.user_id}/profile-pictures`,
          data,
          config
        );
    ...
    async handleSubmitChangePassword() {
      ...
      try {
        await api.put(
          `/partner/users/${this.model.parceiro.user_id}/change-password`,
          this.model.user
        );
