...
export default {
  ...
  methods: {
    async loadMeetings() {
      ...
      try {
        const { data } = await api.get("/associate/meetings/next");
        this.meetings = data;
      } catch (e) {
    ...
    async showMeetModal(meet) {
      ...
      try {
        if (!meet.description) {
          const { data } = await api.get("/associate/meetings/" + meet.id);
    ...
    async handleDeleteComment(id) {
      try {
        await api.delete(`/associate/news/${id}/`);
    ...
    async loadSurveys() {
      try {
        ...
        } = await api.get("/associate/home-surveys");
    ...
    async loadBirthday() {
      try {
        const { data } = await api.get("/associate/users-birthday-stats");
        if (data) {
          this.birthday = data;
        }
      } catch (error) {
        ...
        if (bottomOfWindow && this.meta.current_page < this.meta.last_page) {
          ...
          try {
            const {
              ...
              `/associate/news/?page=${this.meta.current_page + 1}`
            );
            if (data.length) {
    ...
    async handleSetStatus(index) {
      ...
      try {
        await api.patch(`/associate/news/${this.news[index].id}/set-status`, {
    ...
    async handleLike(index) {
      ...
      try {
        await api.put(`/associate/news/${this.news[index].id}/like`);
      } catch (error) {
        this.news[index].likes_count -= 1;
        this.news[index].liked = !this.news[index].liked;
