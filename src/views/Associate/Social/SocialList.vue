  ...
  data() {
    return {
      ...
      status: {
        ...
        hidden: {
          tooltip: "Publicação oculta pelo parceiro",
          name: "Oculto",
          type: "primary",
        },
      },
  ...
  methods: {
    async loadPublications(getParams, targetObjName, loaderRef, errorMsg) {
      ...
      try {
        const {
          data: { data },
        } = await api.get(`/associate/news/user/${0}?${getParams}`);
    ...
    async handleOpenModalView(news, ref) {
      ...
      try {
        const { data } = await api.get(`/associate/news/${news.id}`);
    ...
    openPublicacao(id) {
      this.$router.push(`/parceiro/murais/publicacoes/${id}/editar`);
    },
    openLink(link) {
