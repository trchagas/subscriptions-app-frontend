...
export default {
  ...
  methods: {
    ...
    async handleDeleteComment(id) {
      try {
        await api.delete(`/associate/news/${this.news.id}/comment/${id}`);
    ...
    async handleCommentResponse(id) {
      ...
      try {
        await api.post(`/associate/news/${this.news.id}/comment`, {
    ...
    async handleLike() {
      ...
      try {
        await api.put(`/associate/news/${this.news.id}/like`);
      } catch (error) {
        this.news.likes -= 1;
        this.news.liked = !this.news.liked;
    ...
    async loadPublications(targetObjName, loaderRef, errorMsg) {
      ...
      try {
        const { data } = await api.get(
          `/associate/news/${this.$route.params.id}`
        );
