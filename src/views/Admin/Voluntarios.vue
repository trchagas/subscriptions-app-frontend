        ...
        <div class="row">
          ...
          <div class="col-lg-8 col-md-10">
            <p class="text-white mt-3 mb-5">
              Aqui você pode gerenciar os voluntarios cadastrados.
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="empty" class="row mt-3">
        <div class="col-12 mb-3 text-center">
          <badge type="info" class="bg-white">
            Não existem voluntarios cadastrados até o momento
          </badge>
        </div>
      </div>
      <div class="row text-center mt-1 mb-6">
        <div class="col-12">
          <base-button
            ...
            size="sm"
            type="primary"
            @click="$router.push('/diretoria/usuarios/voluntarios/novo')"
          >
            ...
            <div class="btn-group">
              <base-button
                ...
                :class="{
                  ...
                  'border-success': status === 'active',
                  'bg-dark': status === 'pendent',
                  'border-dark': status === 'pendent',
                }"
              ...
              <base-button
                class="mb-5"
                :class="{
                  'bg-success': status === 'pendent',
                  'border-success': status === 'pendent',
                  'bg-dark': status === 'active',
                  'border-dark': status === 'active',
                }"
                @click="handleStatus('pendent')"
              >
                Usuários Pendentes
              </base-button>
            </div>
    ...
    >
      <div class="row">
        ...
        <div class="col-12 mb-3 mt-2 text-right d-flex justify-content-between">
          ...
          >
          <base-button type="primary" size="sm" @click="loadVolunteers()"
            >Aplicar</base-button
          >
        </div>
        <hr class="my-4" />
...
import VolunteerCard from "./Voluntarios/VolunteerCard.vue";
...
export default {
  ...
  async mounted() {
    this.status = this.$route.query.status === "pendent" ? "pendent" : "active";
  ...
  methods: {
    ...
    handleStatus(type) {
      ...
      this.status = type;
      this.$router.push({
        path: "/diretoria/usuarios/voluntarios",
    ...
    async loadVolunteers() {
      ...
      try {
        const { data } = await api.get(
          `admin/volunteers?status${this.status}&profile_picture=true&name=${this.query.name}&month=${this.query.month}&year=${this.query.year}`
        );
        data.forEach((e) => {
        ...
        this.$toasted.error("Ocorreu um erro ao buscar os voluntários");
      }
    },
  },
};
