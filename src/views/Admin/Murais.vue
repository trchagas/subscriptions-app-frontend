      ...
      <div class="row">
        <div class="col-12 text-center">
          <base-button
            ...
            class="mt--3"
            :custom="true"
            @click="$router.push('/diretoria/murais/publicacoes/nova')"
          >
      ...
      <div class="text-center">
        ...
        <p class="description mt-1 mb-0">
          <span v-if="modalView.notices.date">
            <b>Data: </b>
            {{ formatDate(modalView.notices.date, "LL") }}
          </span>
          <span v-if="modalView.notices.start_hour" class="px-2">
            <b>Hora: </b>
            {{ modalView.notices.start_hour }}
          </span>
        </p>
...
export default {
  ...
  methods: {
    async loadPublications(getParams, targetObjName, loaderRef, errorMsg) {
      ...
      try {
        const { data } = await api.get(`/admin/news?${getParams}`);
    ...
    async handleOpenModalView(notices, ref) {
      ...
      try {
        const { data } = await api.get(`/admin/news/${notices.id}`);
    ...
    openPublicacao(id) {
      this.$router.push(`/diretoria/murais/publicacoes/${id}/editar`);
    },
    openLink(link) {
