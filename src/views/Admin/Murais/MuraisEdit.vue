            ...
            <div ref="publicacao_container" class="card-body vld-parent">
              ...
              <div class="row pl-lg-4">
                <div class="col-lg-12 mb-2">
                  <label class="form-control-label d-block"> Pública </label>
                  <base-switch
                  ...
                  <div>
                    <i
                      :class="`d-inline-block text-sm text-danger 
                      ${model.is_public ? 'visible' : 'invisible'}`"
                    >
                      Uma publicação pública pode ser vista mesmo sem login.
                    </i>
                  </div>
                </div>
                <div class="col-lg-6">
...
export default {
  ...
  data() {
    return {
      ...
      status: {
        ...
        hidden: {
          name: "Oculto  (visível somente para a diretoria)",
          type: "primary",
        },
      },
      types: [
  ...
  async mounted() {
    ...
    try {
      const { data } = await api.get(`/admin/news/${this.$route.params.id}`);
      ...
      if (data) {
        ...
        this.model.is_public = Boolean(data.is_public);
      ...
      this.$router.push("/diretoria/comunidade");
    } finally {
      loader.hide();
    }
  },
  methods: {
    async handleUpdate() {
      ...
      try {
        ...
        await api.put(`/admin/news/${this.model.id}`, data, config);
    ...
    async handleRemove() {
      ...
      try {
        await api.delete(`/admin/news/${this.$route.params.id}`);
