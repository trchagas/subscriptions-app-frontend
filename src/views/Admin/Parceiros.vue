        ...
        <div class="row">
          ...
          <div class="col-lg-8 col-md-10">
            <p class="text-white mt-3 mb-5">
              Aqui você pode gerenciar os parceiros cadastrados.
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="empty" class="row mt-3">
        <div class="col-12 mb-3 text-center">
          <badge type="info" class="bg-white">
            Não existem parceiros cadastrados até o momento
          </badge>
        </div>
      </div>
      <div class="row text-center mt-1 mb-6">
        <div class="col-12">
          <base-button
            ...
            size="sm"
            type="primary"
            @click="$router.push('/diretoria/usuarios/parceiros/novo')"
          >
...
import PartnerCard from "./Parceiros/PartnerCard.vue";
    ...
    async loadAssociates() {
      ...
      try {
        const { data } = await api.get(
          `admin/partners?profile_picture=true&name=${this.query.name}&month=${this.query.month}&year=${this.query.year}`
        );
        data.forEach((e) => {
