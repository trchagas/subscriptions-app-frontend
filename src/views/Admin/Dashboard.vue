                      ...
                      <div class="row mt-0">
                        <div class="col-12">
                          <base-button
                            ...
                            @click="
                              $router.push(
                                `/diretoria/usuarios/${b.user_id}/visualizar`
                              )
                            "
                          >
                            Parabenizar colega
                      ...
                      <div class="row mt-0">
                        <div class="col-12">
                          <base-button
                            ...
                            @click="
                              $router.push(
                                `/colaborador/colaboradores/${b.user_id}/visualizar`
                              )
                            "
                          >
                            Parabenizar colega
            ...
            >
              <div class="card-header border-0">
                ...
                <div v-if="meetings.length" class="">
                  ...
                  <div class="row">
                    <div class="col-12">
                      <router-link class="float-right" to="/diretoria/reunioes">
            ...
            <div class="post-card shadow col-xl-12 col-lg-12 mb-4 card px-2">
              ...
              <div class="btn-group mb-3 d-flex border-group">
                ...
                <base-button
                  ...
                  type=""
                  @click="
                    $router.push(`diretoria/publicacao/${event.id}/visualizar`)
                  "
                >
                ...
                <base-button
                  v-tooltip="
                    news[index].status === 'public'
                      ? 'Oculte uma publicação que pode não estar de acordo.'
                      : 'Deixar publicação visível novamente.'
                  "
                  class="btn btn-white py-1"
                  @click="handleSetStatus(index)"
                >
                  <i
                    v-if="news[index].status === 'public'"
      ...
      <div class="text-center">
        <div class="mt-7 mb--7">
          <base-button
            ...
            class="p-2"
            @click="
              $router.push(`/diretoria/reunioes/${modalMeet.meeting.id}/editar`)
            "
          >
        ...
        <p class="description mt-1 mb-0">
          <b>Disponibilidade:</b>
          <badge v-if="modalMeet.meeting.teacher" class="mx-1" type="success">
            Professores
          </badge>
          <badge v-if="modalMeet.meeting.partner" class="mx-1" type="success">
            Parceiros
          </badge>
          <badge v-if="modalMeet.meeting.associate" class="mx-1" type="success">
            Associados
          </badge>
          <badge v-if="modalMeet.meeting.volunteer" class="mx-1" type="success">
            Voluntários
          </badge>
          <badge v-if="modalMeet.meeting.other" class="mx-1" type="info">
            Outros
          </badge>
        </p>
      </div>
  ...
  methods: {
    async loadMeetings() {
      const loader = this.$loading.show({
        container: this.$refs.news_table,
        width: 90,
        height: 90,
        opacity: 0,
      });
      try {
        const { data } = await api.get("/admin/meetings/next");
        this.meetings = data;
      } catch (e) {
    ...
    async showMeetModal(meet) {
      ...
      try {
        if (!meet.description) {
          const { data } = await api.get("/admin/meetings/" + meet.id);
    ...
    async loadBirthday() {
      try {
        const { data } = await api.get("/admin/users-birthday-stats");
        if (data) {
          this.birthday = data;
        }
      } catch (error) {
        ...
        if (bottomOfWindow && this.meta.current_page < this.meta.last_page) {
          ...
          try {
            const {
              ...
              `/admin/news/?page=${this.meta.current_page + 1}`
            );
            if (data.length) {
    ...
    async handleSetStatus(index) {
      ...
      try {
        await api.patch(`/admin/news/${this.news[index].id}/set-status`, {
    ...
    async handleLike(index) {
      ...
      try {
        await api.put(`/admin/news/${this.news[index].id}/like`);
      } catch (error) {
        this.news[index].likes_count -= 1;
        this.news[index].liked = !this.news[index].liked;
    ...
    handleViewUser(news) {
      this.$router.push(`/diretoria/usuarios/${news.user_id}/visualizar`);
    },
