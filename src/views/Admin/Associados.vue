        ...
        <div class="row">
          ...
          <div class="col-lg-8 col-md-10">
            <p class="text-white mt-3 mb-5">
              Aqui você pode gerenciar os associados cadastrados.
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-if="empty" class="row mt-3">
        <div class="col-12 mb-3 text-center">
          <badge type="info" class="bg-white">
            Não existem associados cadastrados até o momento
          </badge>
        </div>
      </div>
      <div class="row text-center mt-1 mb-6">
        <div class="col-12">
          <base-button
            ...
            size="sm"
            type="primary"
            @click="$router.push('/diretoria/usuarios/associados/novo')"
          >
    ...
    >
      <div class="row">
        ...
        <div class="col-12 mb-3 mt-2 text-right d-flex justify-content-between">
          ...
          >
          <base-button type="primary" size="sm" @click="loadAssociates()"
            >Aplicar</base-button
          >
        </div>
        <hr class="my-4" />
...
import AssociateCard from "./Associados/AssociateCard.vue";
const defaultProfilePicture = "/img/pages/default_profile_picture.png";
import RightSidebar from "@/components/RightSidebar";

export default {
  name: "AdminAssociados",
  components: {
    AssociateCard,
    RightSidebar,
  },
    ...
    async loadAssociates() {
      ...
      try {
        const { data } = await api.get(
          `admin/associates?profile_picture=true&name=${this.query.name}&month=${this.query.month}&year=${this.query.year}`
        );
        data.forEach((e) => {
        ...
        this.$toasted.error("Ocorreu um erro ao buscar os associados");
      }
    },
  },
};
