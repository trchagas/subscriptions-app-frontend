      ...
      <div class="container-fluid d-flex align-items-center">
        <div class="row">
          <div class="col-12 mt-0 mt-md-4 mt-lg-0 d-block">
            <router-link
              :to="`/diretoria/usuarios/voluntarios?status=${$route.query.status}`"
              class="btn-link font-weight-normal text-white"
              title="Voltar para a listagem de voluntarios"
            >
          ...
          <div class="col-lg-7 col-md-10">
            <p class="text-white mt-4 mb-2">
              ...
              do voluntário, o que inclui modificar a foto de perfil e a senha,
              além de controlar a validade da conta do voluntário.
            </p>
          </div>
        </div>
      </div>
                ...
                <div class="col">
                  <div class="text-center mt-6">
                    <h2 class="mb-2 card-title text-orange">
                      {{ model.voluntario.name }}
                    </h2>
                    <div class="h5 font-weight-300 mb-3">
                      <badge
                        v-tooltip="'Setor de atuação do voluntario'"
                        type="info"
                      >
                        {{ model.voluntario.working_area }}
                      </badge>
                    </div>
                    <div v-if="model.voluntario.email">
                      <div class="h5 font-weight-300 mb-1">
                        <email-link :email-address="model.voluntario.email" />
                      </div>
                      <div class="h5 font-weight-300 mb-1">
                        <whatsapp-link :phone-number="model.voluntario.phone" />
                      </div>
                    </div>
                    <hr class="mt-3 mb-4" />
                    <div class="row">
                      <div class="col">
                        <file-select
                          ...
                          title="Selecione a foto de perfil do voluntário"
                          accept="image/*"
                          :mb-size="2"
                        />
                        <br />
            ...
            <template>
              <form ref="voluntario_form" role="form" class="vld-parent">
                ...
                <div class="row ml-2">
                  ...
                  >
                    <badge
                      v-if="model.voluntario.account_validated"
                      type="success"
                      title="A conta do voluntario está validada."
                    >
                    ...
                    <badge
                      v-else
                      type="warning"
                      title="Enquanto a conta não for validada o voluntario não terá acesso as funcionalidades do portal."
                    >
                      <i class="fas fa-info-circle mr-1" /> Conta não validada
                    </badge>
                    <base-button type="info" size="sm" @click="handleTeacher">
                      {{
                        model.voluntario.is_teacher
                    ...
                    <base-button
                      :type="
                        model.voluntario.account_validated ? 'danger' : 'info'
                      "
                      size="sm"
                      @click="handleSubmitToggleStatus()"
                    >
                      <div v-if="model.voluntario.account_validated">
                ...
                <div class="row ml-lg-2">
                  ...
                  <div class="col-lg-6">
                    <base-input label="Identidade (opcional)">
                      <the-mask
                        v-model="model.voluntario.rg"
                        :mask="['XXX.XXX.XXX-X']"
                        class="form-control"
                        placeholder="Digite o RG do voluntário"
                      />
                    </base-input>
                  </div>
                  <div class="col-lg-6">
                ...
                >
                  <div class="col-lg-6">
                    <base-input label="CPF do voluntário">
                      <the-mask
                        v-model="model.voluntario.cpf"
                        :mask="['###.###.###-##']"
                        class="form-control"
                        type="tel"
                  ...
                  <div class="col-lg-6">
                    ...
                    <base-input
                      v-model="model.voluntario.specialty"
                  ...
                  <div class="col-lg-6">
                    ...
                    <base-input
                      v-model="model.voluntario.start_at"
                      addon-left-icon="fas fa-calendar"
                      type="date"
                  ...
                  <div class="col-lg-12">
                    <base-input label="Biografia">
                      <textarea
                        v-model="model.voluntario.short_bio"
                        rows="2"
                        class="form-control"
                        placeholder="Breve biografia do voluntario"
                      />
                    </base-input>
                  </div>
                  <div class="col-lg-12">
                    ...
                    <base-input label="Observações">
                      <textarea
                        v-model="model.voluntario.note"
                        rows="2"
                        class="form-control"
                        placeholder="Observações sobre o voluntario"
                      />
                    </base-input>
                  </div>
                </div>
                ...
                <div class="row">
                  ...
                  <div
                    v-if="model.voluntario.account_status === 'pendent'"
                    class="col ml-1 text-center"
                  >
                    <h6 class="heading-small text-muted">
                      <base-button
                        type="info"
    ...
    >
      ...
      <div ref="modal_confirmation_delete" class="vld-parent">
        <div class="py-3 text-center">
          ...
          <h4 class="heading mt-4">
            Você realmente deseja excluir o voluntario
            <span class="text-warning">
              {{ model.voluntario.name }}
            </span>
            ?
      ...
      <template slot="footer">
        ...
        <base-button
          ...
          @click="handleSubmitRemoveVoluntario"
        >
          Confirmar exclusão
        </base-button>
      </template>
    ...
    >
      ...
      <div ref="modal_confirmation_delete" class="vld-parent">
        <div class="py-3 text-center">
          <i class="fa fa-trash fa-3x" />
          <h4 class="heading mt-4">
            Você deseja validar o voluntario
          ...
          <p class="mt-3 mb-0">
            Com isso, o usuário terá permissões de voluntário.
          </p>
        </div>
      </div>
      <template slot="footer">
...
export default {
  ...
  computed: {
    getVoluntarioProfilePicturePath() {
      ...
      const profilePicture = this.model.voluntario.profile_picture;
  ...
  watch: {
    other() {
      if (!this.registeredTypes.includes(this.model.voluntario.working_area))
        this.extraType = this.model.voluntario.working_area;
      else this.extraType = "";
    },
  },
  async mounted() {
    ...
    try {
      this.model.voluntario.user_id = this.$route.params.id;
      const { data } = await api.get(
        `/admin/volunteers/${this.model.voluntario.user_id}`
      );
      ...
      data.start_at = data.start_at
        ...
        : "";
      Object.assign(this.model.voluntario, data);
      this.model.voluntario.email = data.user.email;
      loaderForm.hide();
  ...
  methods: {
    handleTeacher() {
      const is_teacher = this.model.voluntario.is_teacher;
      const id = this.model.voluntario.user_id;
      if (is_teacher)
        this.$router.push(`/diretoria/usuarios/professores/${id}/editar`);
      else
        this.$router.push({
          path: "/diretoria/usuarios/professores/novo",
          query: { user_id: id },
        });
    },
    async handleSubmitForm(e) {
      ...
      const voluntarioData = this.model.voluntario;
      for (const key in voluntarioData) {
        if (this.requiredFields.includes(key) && !voluntarioData[key]) {
          this.error =
    ...
    async handleSubmitProfilePicture() {
      ...
      try {
        await api.post(
          `/admin/users/${this.model.voluntario.user_id}/profile-pictures`,
          data,
          config
        );
        ...
        if (profile_pictureData)
          this.model.voluntario.profile_picture = profile_pictureData;
    ...
    async handleConfirmVoluntario() {
      ...
      try {
        await api.put(
          `/admin/volunteers/${this.model.voluntario.user_id}/validate`
        );
        this.$toasted.show("Voluntario validade com sucesso");
        this.model.voluntario.account_status = "updating";
        this.model.voluntario.account_validated = true;
        this.modals.modalConfirmation = false;
      } catch (e) {
        this.$toasted.error("Ocorreu um erro ao atualizar o voluntario");
      } finally {
        loader.hide();
      }
    },
    async handleSubmitRemoveVoluntario() {
      const loader = this.$loading.show({
        container: this.$refs.modal_confirmation_delete,
        width: 160,
        height: 160,
      ...
      try {
        await api.delete(`/admin/volunteers/${this.model.voluntario.user_id}`);
        this.$toasted.show("Voluntario removido com sucesso");
        this.$router.back();
      } catch (e) {
        this.$toasted.error("Ocorreu um erro ao remover o voluntario");
      } finally {
        loader.hide();
      }
    },
    async handleSubmitChangePassword() {
      ...
      try {
        await api.put(
          `/admin/users/${this.model.voluntario.user_id}/change-password`,
          this.model.user
        );
        ...
        const genericError =
          "Ocorreu um erro ao atualizar a senha do voluntario.";
        const hasErrorResponseData = e.response && e.response.data;
        let customError = genericError;
        if (hasErrorResponseData) {
    ...
    async handleSubmitToggleStatus() {
      ...
      try {
        const status = {
          account_validated: !this.model.voluntario.account_validated,
        };
        const {
          data: { account_validated },
        } = await api.put(
          `/admin/volunteers/${this.model.voluntario.user_id}/update-status`,
          status
        );
        this.model.voluntario.account_validated = account_validated;
