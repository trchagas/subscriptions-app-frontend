      ...
      <div class="container-fluid d-flex align-items-center">
        <div class="row">
          <div class="col-12 mt-0 mt-md-4 mt-lg-0 d-block">
            <router-link
              :to="`/admin/turmas/${class_id}/materiais`"
                        ...
                        <template slot-scope="{ row, index }">
                          ...
                          <td>
                            ...
                            <router-link
                              class="ml-1"
                              title="Abrir aluno"
                              :to="`/admin/usuarios/${row.student.user_id}/visualizar`"
                            >
                              <i class="fas fa-eye" />
                            </router-link>
                          </td>
...
export default {
  ...
  methods: {
    async loadClasse() {
      ...
      try {
        const { data } = await api.get(`/admin/classes/${this.class_id}`);
        this.classe = data;
      } catch (e) {
        this.$toasted.error(
          "Ocorreu um erro ao recuperar as informações da turma"
        );
        this.$router.push(`/admin/turmas/${this.class_id}/materiais`);
      }
      loader.hide();
    },
    async loadAnswers() {
      ...
      try {
        const {
          ...
          `/admin/classes/${this.class_id}/materials/${this.material_id}/answers`
        );
        if (!questions.length) {
          this.empty = true;
          return;
