      ...
      <div class="container-fluid d-flex align-items-center">
        <div class="row">
          <div class="col-12 mt-0 mt-md-4 mt-lg-0 d-block">
            <a
              title="Voltar para a listagem de cursos"
  ...
  methods: {
    ...
    async generateAttendanceReport() {
      try {
        const { data } = await api.get(
          `/admin/classes/${this.$route.params.id}/attendance-report?startDate=${this.startDate}&endDate=${this.endDate}`
        );
    ...
    async loadCourse() {
      ...
      try {
        const { data: classe } = await api.get(
          `/admin/classes/${this.$route.params.id}`
        );
        this.classe = classe;
      } catch (e) {
        this.$toasted.error(
    ...
    async loadRanking() {
      ...
      try {
        ...
        } = await api.get(`/admin/classes/${this.$route.params.id}/ranking`);
        this.lessons_count = lessons_count;
        if (!attendance_ranking.length) {
          this.empty = true;
        } else {
          attendance_ranking.forEach((collaborator) => {
            if (!lessons_count) {
              collaborator.completion = 0;
            }
            collaborator.completion = Number(
              ((collaborator.attendance / lessons_count) * 100).toFixed(2)
            );
          });
