...
export default {
  ...
  data() {
    return {
      ...
      status: {
        ...
        hidden: {
          name: "Oculto  (visível somente para o aluno)",
          type: "primary",
        },
      },
      types: [
  ...
  async mounted() {
    ...
    try {
      const { data } = await api.get(`/student/news/${this.$route.params.id}`);
      ...
      this.$router.push("/aluno/comunidade");
    } finally {
      loader.hide();
    }
  },
  methods: {
    async handleUpdate() {
      ...
      try {
        ...
        await api.put(`/student/news/${this.model.id}`, data, config);
    ...
    async handleRemove() {
      ...
      try {
        await api.delete(`/student/news/${this.$route.params.id}`);
