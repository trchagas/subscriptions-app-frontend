              ...
              <form ref="aluno_form" role="form" class="vld-parent">
                ...
                <div class="row ml-lg-2">
                  <div class="col-lg-6">
                    ...
                    >
                      <p>{{ model.aluno.name }}</p>
                    </base-input>
                  </div>
                  <div class="col-lg-6">
                    <base-input label="Sexo biológico">
                      <p>{{ genderLabels[model.aluno.gender] }}</p>
                    </base-input>
                  </div>
                  <div class="col-lg-6">
                    <base-input label="Área" placeholder="Digite a atuação">
                      <p>{{ model.aluno.working_area }}</p>
                    </base-input>
                  </div>
                  <div class="col-lg-6">
                    <base-input
                    ...
                    >
                      <p v-if="model.aluno.specialty">
                        {{ model.aluno.specialty }}
                      </p>
                  ...
                  <div class="col-lg-6">
                    <base-input label="Identidade (RG/CPF)">
                      <p v-if="model.aluno.rg_cpf">
                        {{ formatRgCpf(model.aluno.rg_cpf) }}
                      </p>
                  ...
                  <div class="col-lg-6">
                    <base-input label="Número de Identificação Social">
                      <p v-if="model.aluno.social_identification_number">
                        {{
                          formatNis(model.aluno.social_identification_number)
                        }}
                      </p>
                      <p v-else>Não informado</p>
                    </base-input>
                  ...
                  <div class="col-lg-6">
                    ...
                    >
                      <p v-if="model.aluno.birth">
                        {{ formatDate(model.aluno.birth) }}
                      </p>
                ...
                >
                  <div v-if="type === 'default'" class="col-lg-6">
                    <base-input
                      ...
                      placeholder="Digite o nome"
                    >
                      <p v-if="model.aluno.responsible_name">
                        {{ model.aluno.responsible_name }}
                      </p>
                      <p v-else>Não informado</p>
                    </base-input>
                  </div>
                  <div v-if="type === 'default'" class="col-lg-6">
                    <base-input label="Identidade do responsável (RG/CPF)">
                      <p v-if="model.aluno.responsible_rg_cpf">
                        {{ model.aluno.responsible_rg_cpf }}
                      </p>
                      <p v-else>Não informada</p>
                    </base-input>
                  </div>
                ...
                <div ref="complementar_info_container" class="col-lg-12 mb-4">
                  <ul class="list-group list-group-flush">
                    ...
                    >
                      ...
                      <a
                        class="btn btn-primary"
                        :href="`${files.application_form.url}`"
                        target="_blank"
                      >
                        <i class="fas fa-eye mr-1" />
                        Visualizar
                    ...
                    >
                      ...
                      <a
                        v-if="files.responsible_identity"
                        class="btn btn-primary"
                        :href="`${files.responsible_identity.url}`"
                        target="_blank"
                      >
                        <i class="fas fa-eye mr-1" />
                        Visualizar
                    ...
                    >
                      ...
                      <a
                        v-if="files.school_attendance"
                        class="btn btn-primary"
                        :href="`${files.school_attendance.url}`"
                        target="_blank"
                      >
                        <i class="fas fa-eye mr-1" />
                        Visualizar
...
export default {
  ...
  computed: {
    getAlunoProfilePicturePath() {
      ...
      const profilePicture = this.model.aluno.profile_picture;
  ...
  async mounted() {
    ...
    try {
      ...
      this.files.identity = data.identity_file;
      this.files.address = data.address_file;
      this.files.application_form = data.application_form_file;
      this.files.responsible_identity = data.responsible_identity_file;
      this.files.school_attendance = data.school_attendance_file;
  ...
  methods: {
    async handleSubmitForm(e) {
      ...
      try {
        await api.put(`/student/students/${this.model.aluno.user_id}`, {
          short_bio: this.model.aluno.short_bio,
          phone: this.model.aluno.phone,
        });
    ...
    async handleSubmitProfilePicture() {
      ...
      try {
        await api.post(
          `/student/users/${this.model.aluno.user_id}/profile-pictures`,
          data,
          config
        );
    ...
    async handleSubmitChangePassword() {
      ...
      try {
        await api.put(
          `/student/users/${this.model.aluno.user_id}/change-password`,
          this.model.user
        );
    ...
    formatRgCpf(cpf) {
      ...
      return cpf.length === 11
        ? cpf.replace(/(\w{3})(\w{3})(\w{3})(\w{2})/, "$1.$2.$3-$4")
        : cpf.replace(/(\w{2})(\w{3})(\w{3})(\w{1})$/, "$1.$2.$3-$4");
    },
    formatDate(date) {
