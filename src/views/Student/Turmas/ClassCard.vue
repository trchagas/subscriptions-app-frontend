  ...
  <card-root :title="turma.title" :image-url="turma.background_url">
    ...
    <template slot="footer">
      <div class="row">
        <div class="col-12">
          <router-link
            v-tooltip="'Clique para abrir a turma e ver os materiais'"
            :to="'/aluno/turmas/' + turma.id + '/visualizar'"
            class="btn btn-primary btn-radius btn-block"
          >
            <i class="fas fa-book text-dark-green mr-2" />Abrir turma
          </router-link>
        </div>
      </div>
    </template>
...
export default {
  ...
  components: { CardRoot },
  props: { turma: { type: Object, required: true } },
  methods: {
