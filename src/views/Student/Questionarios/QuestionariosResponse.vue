                ...
                <div class="col-12 text-center">
                  <base-alert type="success">
                    ...
                    <base-button
                      ...
                      class="mt-2"
                      size="sm"
                      @click="$router.push('/aluno/questionarios')"
  ...
  methods: {
    async loadSurvey() {
      const { data } = await api.get(
        `/student/surveys/${this.$route.params.id}`
      );
    ...
    async handleSubmit() {
      ...
      try {
        ...
        if (!this.survey.response) {
          await api.post(`/student/surveys/${this.survey.id}`, {
            answers: formattedAnswers,
            is_anonymous: model.is_anonymous,
          });
        } else {
          await api.put(
            `/student/surveys/${this.survey.id}/responses/${this.survey.response.id}`,
            {
              answers: formattedAnswers,
              is_anonymous: model.is_anonymous,
            }
