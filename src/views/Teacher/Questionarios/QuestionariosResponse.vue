                ...
                <div class="col-12 text-center">
                  <base-alert type="success">
                    ...
                    <base-button
                      ...
                      class="mt-2"
                      size="sm"
                      @click="$router.push('/professor/questionarios')"
  ...
  methods: {
    async loadSurvey() {
      const { data } = await api.get(
        `/teacher/surveys/${this.$route.params.id}`
      );
    ...
    async handleSubmit() {
      ...
      try {
        ...
        if (!this.survey.response) {
          await api.post(`/teacher/surveys/${this.survey.id}`, {
            answers: formattedAnswers,
            is_anonymous: model.is_anonymous,
          });
        } else {
          await api.put(
            `/teacher/surveys/${this.survey.id}/responses/${this.survey.response.id}`,
            {
              answers: formattedAnswers,
              is_anonymous: model.is_anonymous,
            }
