                      ...
                      <div class="row mt-0">
                        <div class="col-12">
                          <base-button
                            ...
                            @click="
                              $router.push(
                                `/professor/usuarios/${b.user_id}/visualizar`
                              )
                            "
                          >
                            Parabenizar colega
                        ...
                        <div class="col-12">
                          <div v-if="s.responses.responses_count" class="">
                            <base-button
                              ...
                              @click="
                                $router.push(
                                  `/professor/questionarios/${s.id}/respostas`
                                )
                              "
                            >
            ...
            <div class="post-card shadow col-xl-12 col-lg-12 mb-4 card px-2">
              ...
              <div class="btn-group mb-3 d-flex border-group">
                ...
                <base-button
                  ...
                  type=""
                  @click="
                    $router.push(`professor/publicacao/${event.id}/visualizar`)
                  "
                >
...
export default {
  ...
  methods: {
    async loadMeetings() {
      ...
      try {
        const { data } = await api.get("/teacher/meetings/next");
        this.meetings = data;
      } catch (e) {
    ...
    async showMeetModal(meet) {
      ...
      try {
        if (!meet.description) {
          const { data } = await api.get("/teacher/meetings/" + meet.id);
    ...
    async loadSurveys() {
      try {
        ...
        } = await api.get("/teacher/home-surveys");
    ...
    async loadBirthday() {
      try {
        const { data } = await api.get("/teacher/users-birthday-stats");
        if (data) {
          this.birthday = data;
        }
      } catch (error) {
        ...
        if (bottomOfWindow && this.meta.current_page < this.meta.last_page) {
          ...
          try {
            const {
              ...
              `/teacher/news/?page=${this.meta.current_page + 1}`
            );
            if (data.length) {
    ...
    async handleSetStatus(index) {
      ...
      try {
        await api.patch(`/teacher/news/${this.news[index].id}/set-status`, {
    ...
    async handleLike(index) {
      ...
      try {
        await api.put(`/teacher/news/${this.news[index].id}/like`);
      } catch (error) {
        this.news[index].likes_count -= 1;
        this.news[index].liked = !this.news[index].liked;
    ...
    handleViewUser(news) {
      this.$router.push(`/professor/usuarios/${news.user_id}/visualizar`);
    },
