        ...
        <div class="row">
          ...
          <div class="col-lg-7 col-md-10">
            <p class="text-white mt-3 mb-5">
              Aqui você pode gerenciar as notícias, eventos, treinamentos ou
...
export default {
  ...
  data() {
    return {
      ...
      status: {
        ...
        hidden: {
          name: "Oculto  (visível somente para o professor)",
          type: "primary",
        },
      },
      types: [
  ...
  async mounted() {
    ...
    try {
      const { data } = await api.get(`/teacher/news/${this.$route.params.id}`);
      ...
      this.$router.push("/professor/comunidade");
    } finally {
      loader.hide();
    }
  },
  methods: {
    async handleUpdate() {
      ...
      try {
        ...
        await api.put(`/teacher/news/${this.model.id}`, data, config);
    ...
    async handleRemove() {
      ...
      try {
        await api.delete(`/teacher/news/${this.$route.params.id}`);
