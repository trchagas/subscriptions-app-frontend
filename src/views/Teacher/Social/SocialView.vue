  ...
  <div>
    ...
    <div class="container-fluid mt--5">
      ...
      >
        ...
        <div
          v-for="(comment, cindex) in news.comments"
          ref="news_table"
          :key="cindex"
        >
          <div class="mb-0">
...
export default {
  ...
  methods: {
    ...
    async handleDeleteComment(id) {
      try {
        await api.delete(`/teacher/news/${this.news.id}/comment/${id}`);
    ...
    async handleCommentResponse(id) {
      ...
      try {
        await api.post(`/teacher/news/${this.news.id}/comment`, {
    ...
    async handleLike() {
      ...
      try {
        await api.put(`/teacher/news/${this.news.id}/like`);
      } catch (error) {
        this.news.likes -= 1;
        this.news.liked = !this.news.liked;
    ...
    async loadPublications(targetObjName, loaderRef, errorMsg) {
      ...
      try {
        const { data } = await api.get(
          `/teacher/news/${this.$route.params.id}`
        );
