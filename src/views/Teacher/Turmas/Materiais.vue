      ...
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="text-center mb-4">
            <base-button
              v-tooltip="'Clique para cadastrar um novo material'"
              :custom="true"
              size="sm"
              type="primary"
              class="mb-3"
              @click="
                $router.push(
                  '/professor/turmas/' + $route.params.id + '/materiais/novo'
                )
              "
            >
              <i class="fas fa-plus mr-1" /> Cadastrar novo material
            </base-button>
          </div>
          <div class="row">
            <div class="col-lg-12">
              <div class="float-right">
                <base-button
                  v-tooltip.bottom="'Gerenciar seções'"
                  type="btn btn-white"
                  class="mb-3 text-default"
                  @click="
                    $router.push(
                      `/professor/turmas/${$route.params.id}/materiais/secoes`
                    )
                  "
                >
                  <i class="fas fa-layer-group fa-lg mr-2" /> Seções
                </base-button>
                <span
                  v-tooltip.bottom="
                    'Ordenar/alterar ordem de exibição dos materiais  da turma.'
                  "
                >
                  <base-button
                    type="btn btn-white"
                    class="mb-3 text-default"
                    :disabled="totalMaterials <= 1"
                    @click="
                      $router.push(
                        `/professor/turmas/${$route.params.id}/materiais/ordenar`
                      )
                    "
                  >
                    <i class="fas fa-arrows-alt-v fa-lg mr-2" /> Ordenar
                  </base-button>
                </span>
              </div>
            </div>
