          ...
          >
            ...
            <template slot-scope="{ row, index }">
              ...
              <td>
                ...
                <base-button
                  v-tooltip.bottom="'Excluir material'"
                  type="white"
                  size="sm"
                  class="text-danger"
                ...
                <router-link
                  v-tooltip.bottom="'Editar material'"
                  :to="`/professor/turmas/${$route.params.id}/materiais/${row.id}/editar`"
...
export default {
  ...
  methods: {
    ...
    async handleRemoveMaterial() {
      ...
      try {
        const removeId = this.modalRemove.material.id;
        await api.delete(
          `/teacher/classes/${this.$route.params.id}/materials/${removeId}`
        );
        this.$toasted.show("Material removido da turma com sucesso!");
        this.modalRemove.show = false;
        const index = this.material.findIndex((m) => m.id === removeId);
    ...
    handleOpenModalRemove(material) {
      this.modalRemove.error = "";
      this.modalRemove.material = material;
    ...
    handleOpenAnswers(material) {
      if (material.content === "evaluation") {
        this.$router.push(
          `/professor/turmas/${material.class_id}/materiais/${material.id}/respostas`
        );
      }
    },
  },
