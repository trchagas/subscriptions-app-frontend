        ...
        <div class="row">
          <div class="col-lg-8 col-md-10">
            <p class="text-white mt-3 mb-5">
              Aqui você pode gerenciar as turmas cadastradas.
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="row text-center mt-1 mb-6">
        <div class="col-12">
          <base-button
            ...
            size="sm"
            type="primary"
            @click="$router.push('/professor/turmas/novo')"
          >
...
export default {
  ...
  computed: {
    yearList() {
      const companyYear = new Date(this.teacher_createdAt).getFullYear();
  ...
  async mounted() {
    ...
    this.teacher_createdAt = data.createdAt;
  },
  methods: {
    async cleanFilters() {
      this.query.title = "";
    ...
    async loadClasses() {
      ...
      try {
        const { data } = await api.get(
          `/teacher/classes?title=${this.query.title}&month=${this.query.month}&year=${this.query.year}`
        );
